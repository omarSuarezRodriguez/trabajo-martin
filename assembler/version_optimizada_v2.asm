...
# Version optimizada: se elimina el riesgo Load-Use reordenando una instruccion
...
loop:
    ...
    addu $t9, $s1, $t4    # OPTIMIZACION: subi esta linea aqui. Calcula la direccion de Y[i]
                          # y no depende de $t6, asi que aprovecha el ciclo que antes se perdia

    mul $t7, $t6, $t0     # multiplico X[i] por A. Ahora $t6 ya esta disponible, no hay parada
    addu $t8, $t7, $t1    # le sumo B al resultado anterior, ya tengo el valor de Y[i]

    sw $t8, 0($t9)        # guardo el resultado en Y[i], usando la direccion que calcule arriba
